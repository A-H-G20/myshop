...
class UserManager {
    ...
    public function getAllUsers($page = 1, $limit = 20, $search = '', $sortBy = 'id', $sortOrder = 'DESC', $roleFilter = 'user') {
        $offset = ($page - 1) * $limit;
    ...
    public function createUser($userData) {
        ...
        try {
            ...
            $role = isset($userData['role']) && !empty($userData['role']) ? $userData['role'] : 'user';
    ...
    public function getUserStats() {
        ...
        $monthQuery = "SELECT COUNT(*) as monthly FROM users WHERE role = 'user' AND created_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH)";
        $result = $this->conn->query($monthQuery);
        $stats['monthly_users'] = $result->fetch_assoc()['monthly'];

        $activeQuery = "SELECT COUNT(DISTINCT u.id) as active FROM users u INNER JOIN orders o ON u.id = o.user_id WHERE u.role = 'user'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validateCSRF($_POST['csrf_token'])) {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'create') {
            ...
            $userManager->createUser($userData); // redirects to user.php
        }
